...
import layout from '../../assets/world/archive-city-v3-layout.json';
import runtimeManifest from '../../assets/runtime/v3/archive-city-v3-manifest.json';
import type { District } from './types';

type RuntimeOptions = { host: HTMLElement; status: HTMLElement; select: (text: string) => void; runtimeBase: string };
...
export function createWorldViewer(options: RuntimeOptions): WorldViewer {
  const { host, status, select, runtimeBase } = options;
  ...
  const loader=new GLTFLoader(); const loaded=new Map<string,THREE.Group>(); const pathById=new Map(runtimeManifest.districts.map((district)=>[district.id,district.runtimePath]));
  const loadDistrict=(id:string)=>{ if(!runtimeBase||id==='city'||loaded.has(id)) return; const path=pathById.get(id); if(!path) return; status.textContent=`LOADING ${id.toUpperCase()} DETAIL…`; loader.load(`${runtimeBase.replace(/\/$/,'')}/${path.replace(/^assets\//,'')}`,(gltf)=>{gltf.scene.name=`Runtime_${id}`;scene.add(gltf.scene);loaded.set(id,gltf.scene);status.textContent=`${id.toUpperCase()} DETAIL READY`;},undefined,()=>{status.textContent=`RUNTIME UNAVAILABLE · ${id}`;}); };

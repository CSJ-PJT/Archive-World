...
function finalizeHeroMaterials(root:THREE.Object3D,performance=false){
 root.traverse(child=>{if(!(child instanceof THREE.Mesh)||Array.isArray(child.material))return;const current=child.material,name=current.name.toLowerCase();
  ...
  if(name.includes('shallow-water'))child.material=pooledHeroMaterial(`water-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:'hero-water-performance',color:0x1e5962,roughness:.36,metalness:.03,transparent:false}):new THREE.MeshPhysicalMaterial({name:'hero-water-cinematic',color:0x286873,roughness:.22,metalness:.03,transmission:.18,thickness:.42,ior:1.333,transparent:true,opacity:.88,depthWrite:true,side:THREE.DoubleSide,clearcoat:.65,clearcoatRoughness:.16}));
  else if(name.includes('frontage-glass')||name.includes('pavilion-glass'))child.material=pooledHeroMaterial(`frontage-glass-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:'frontage-glass',color:0x315c65,roughness:.26,metalness:.08}):new THREE.MeshPhysicalMaterial({name:'frontage-glass',color:0x456f76,roughness:.18,metalness:.03,transmission:.32,transparent:true,opacity:.58,depthWrite:false,clearcoat:.55,clearcoatRoughness:.18}));
  ...
  else if(name.includes('blue-gray-glass'))child.material=pooledHeroMaterial(`blue-gray-glass-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:'blue-gray-glass',color:0x183943,roughness:.28,metalness:.12}):new THREE.MeshPhysicalMaterial({name:'blue-gray-glass',color:0x183943,roughness:.18,metalness:.15,clearcoat:.6,clearcoatRoughness:.2}));
  else if(name.includes('archive-warm-stone'))child.material=pooledHeroMaterial('archive-warm-stone',()=>proceduralSurface(new THREE.MeshStandardMaterial({name:'archive-warm-stone',color:0xa99f8d,roughness:.69}),'archive-warm-stone',.075,.16));
  else if(name.includes('ledger-limestone'))child.material=pooledHeroMaterial('ledger-limestone',()=>proceduralSurface(new THREE.MeshStandardMaterial({name:'ledger-limestone',color:0x989995,roughness:.63}),'ledger-limestone',.065,.18));
  else if(name.includes('ledger-granite'))child.material=pooledHeroMaterial('ledger-granite',()=>proceduralSurface(new THREE.MeshStandardMaterial({name:'ledger-granite',color:0x454b4d,roughness:.52}),'ledger-granite',.045,.28));
  ...
  else if(name.includes('promenade-paver'))child.material=pooledHeroMaterial('promenade-paver',()=>proceduralSurface(new THREE.MeshStandardMaterial({name:'promenade-paver',color:0x747a76,roughness:.87}),'promenade-paver',.055,.28));
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const skyUniforms={topColor:{value:new THREE.Color(0x7695aa)},bottomColor:{value:new THREE.Color(0xd9d7cf)},offset:{value:24.0},exponent:{value:.72}};
 ...
 const hemi=new THREE.HemisphereLight(0xe7eef2,0x27332f,.60);scene.add(hemi);
 const sun=new THREE.DirectionalLight(0xffd7ac,3.15);sun.position.set(-520,760,-620);sun.castShadow=!performanceMode&&!!manifest.heroZones?.length;sun.shadow.mapSize.set(1536,1536);sun.shadow.camera.left=-260;sun.shadow.camera.right=260;sun.shadow.camera.top=220;sun.shadow.camera.bottom=-220;sun.shadow.camera.near=120;sun.shadow.camera.far=1500;sun.shadow.bias=-.00035;sun.target.position.set(-250,0,0);scene.add(sun,sun.target);
 const coolFill=new THREE.DirectionalLight(0xa9c8d6,.42);coolFill.position.set(480,260,520);coolFill.target.position.set(-250,20,0);scene.add(coolFill,coolFill.target);
 ...
 animate();
 const setTime=(mode:'day'|'dusk'|'night')=>{if(mode==='night'){skyUniforms.topColor.value.set(0x07111d);skyUniforms.bottomColor.value.set(0x24303b);skyUniforms.exponent.value=.86;scene.fog=new THREE.Fog(0x111b2d,750,1900);scene.environmentIntensity=.24;renderer.toneMappingExposure=.91;hemi.intensity=.26;sun.intensity=.10;coolFill.intensity=.12;setHeroMaterialState(true);plazaLights.forEach(x=>x.intensity=performanceMode?0:75);heroLights.forEach(x=>x.intensity=performanceMode?0:95);heroRouteLights.forEach(x=>x.intensity=performanceMode?0:24);heroBridgeLights.forEach(x=>x.intensity=performanceMode?0:30);streamLights.forEach(x=>x.light.intensity=performanceMode?0:x.intensity);}else if(mode==='dusk'){skyUniforms.topColor.value.set(0x574d6b);skyUniforms.bottomColor.value.set(0xd39b78);skyUniforms.exponent.value=.68;scene.fog=new THREE.Fog(0x8a7a83,850,2050);scene.environmentIntensity=.52;renderer.toneMappingExposure=.98;hemi.intensity=.58;sun.intensity=.88;coolFill.intensity=.28;setHeroMaterialState(true);plazaLights.forEach(x=>x.intensity=performanceMode?0:32);heroLights.forEach(x=>x.intensity=performanceMode?0:48);heroRouteLights.forEach(x=>x.intensity=performanceMode?0:12);heroBridgeLights.forEach(x=>x.intensity=performanceMode?0:15);streamLights.forEach(x=>x.light.intensity=performanceMode?0:x.intensity*.45);}else{skyUniforms.topColor.value.set(0x7695aa);skyUniforms.bottomColor.value.set(0xd9d7cf);skyUniforms.exponent.value=.72;scene.fog=new THREE.Fog(0xc5d0d3,1200,2600);scene.environmentIntensity=.68;renderer.toneMappingExposure=1.02;hemi.intensity=.60;sun.intensity=3.15;coolFill.intensity=.42;setHeroMaterialState(false);plazaLights.forEach(x=>x.intensity=0);heroLights.forEach(x=>x.intensity=0);heroRouteLights.forEach(x=>x.intensity=0);heroBridgeLights.forEach(x=>x.intensity=0);streamLights.forEach(x=>x.light.intensity=0);}};

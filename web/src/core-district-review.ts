...
function proceduralWaterMaterial(performance=false){
 const water=new THREE.MeshStandardMaterial({name:performance?'hero-water-performance':'hero-water-cinematic',color:0x0d515d,roughness:performance?.48:.34,metalness:.02,transparent:false});
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const hemi=new THREE.HemisphereLight(0xeaf3f5,0x29322d,.34);scene.add(hemi);
 const sun=new THREE.DirectionalLight(0xffe2bc,4.25);sun.position.set(-510,510,-420);sun.castShadow=!performanceMode&&!!manifest.heroZones?.length;sun.shadow.mapSize.set(performanceMode?1024:4096,performanceMode?1024:4096);sun.shadow.camera.left=-170;sun.shadow.camera.right=170;sun.shadow.camera.top=145;sun.shadow.camera.bottom=-145;sun.shadow.camera.near=80;sun.shadow.camera.far=1300;sun.shadow.bias=-.00018;sun.target.position.set(-250,0,0);scene.add(sun,sun.target);
 ...
 for(const family of manifest.families){const source=loaded.get(family.id);if(!source)continue;source.updateMatrixWorld(true);const items=manifest.instances.filter(item=>item.familyId===family.id&&(!performanceMode||!manifest.heroZones?.length||Math.hypot(item.position[0]+250,item.position[2])<520)),buckets=new Map<string,{material:THREE.Material;geometries:THREE.BufferGeometry[]}>();source.traverse(child=>{if(!(child instanceof THREE.Mesh)||Array.isArray(child.material))return;const key=child.material.uuid,bucket=buckets.get(key)??{material:child.material as THREE.Material,geometries:[] as THREE.BufferGeometry[]};bucket.geometries.push(child.geometry.clone().applyMatrix4(child.matrixWorld));buckets.set(key,bucket);});for(const [materialId,bucket] of buckets){const geometry=mergeGeometries(bucket.geometries,false);if(!geometry)throw new Error(`merge failure ${family.id}/${materialId}`);supportEntries.push({familyId:family.id,material:bucket.material,geometry,items});}}
 ...
 animate();
 const setTime=(mode:'day'|'dusk'|'night')=>{if(mode==='night'){skyUniforms.topColor.value.set(0x07111d);skyUniforms.bottomColor.value.set(0x24303b);skyUniforms.exponent.value=.86;scene.fog=new THREE.Fog(0x111b2d,750,1900);scene.environmentIntensity=.24;renderer.toneMappingExposure=.98;hemi.intensity=.22;sun.intensity=.08;coolFill.intensity=.12;setHeroMaterialState(true);plazaLights.forEach(x=>x.intensity=performanceMode?0:32);heroLights.forEach(x=>x.intensity=performanceMode?0:28);heroFrontageLights.forEach((x,index)=>x.intensity=performanceMode?0:index%2?16:24);heroRouteLights.forEach(x=>x.intensity=performanceMode?0:16);heroPromenadeLights.forEach(x=>x.intensity=performanceMode?0:19);heroBridgeLights.forEach(x=>x.intensity=performanceMode?0:18);streamLights.forEach(x=>x.light.intensity=performanceMode?0:x.intensity*.48);}else if(mode==='dusk'){skyUniforms.topColor.value.set(0x574d6b);skyUniforms.bottomColor.value.set(0xd39b78);skyUniforms.exponent.value=.68;scene.fog=new THREE.Fog(0x8a7a83,850,2050);scene.environmentIntensity=.38;renderer.toneMappingExposure=.94;hemi.intensity=.34;sun.intensity=1.05;coolFill.intensity=.14;setHeroMaterialState(true);plazaLights.forEach(x=>x.intensity=performanceMode?0:24);heroLights.forEach(x=>x.intensity=performanceMode?0:24);heroFrontageLights.forEach((x,index)=>x.intensity=performanceMode?0:index%2?10:16);heroRouteLights.forEach(x=>x.intensity=performanceMode?0:11);heroPromenadeLights.forEach(x=>x.intensity=performanceMode?0:14);heroBridgeLights.forEach(x=>x.intensity=performanceMode?0:12);streamLights.forEach(x=>x.light.intensity=performanceMode?0:x.intensity*.32);}else{skyUniforms.topColor.value.set(0x365f85);skyUniforms.bottomColor.value.set(0xd6d1c7);skyUniforms.exponent.value=.72;scene.fog=new THREE.Fog(0xaab9bb,1650,3100);scene.environmentIntensity=.36;renderer.toneMappingExposure=.96;hemi.intensity=.34;sun.intensity=4.25;coolFill.intensity=.16;setHeroMaterialState(false);plazaLights.forEach(x=>x.intensity=0);heroLights.forEach(x=>x.intensity=0);heroFrontageLights.forEach(x=>x.intensity=0);heroRouteLights.forEach(x=>x.intensity=0);heroPromenadeLights.forEach(x=>x.intensity=0);heroBridgeLights.forEach(x=>x.intensity=0);streamLights.forEach(x=>x.light.intensity=0);}};

...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const skyUniforms={topColor:{value:new THREE.Color(0x4f7998)},bottomColor:{value:new THREE.Color(0xe2ded2)},offset:{value:24.0},exponent:{value:.72}};
 ...
 const hemi=new THREE.HemisphereLight(0xeaf3f5,0x29322d,.40);scene.add(hemi);
 const sun=new THREE.DirectionalLight(0xffe2bc,3.80);sun.position.set(-510,510,-420);sun.castShadow=!performanceMode&&!!manifest.heroZones?.length;sun.shadow.mapSize.set(performanceMode?1024:4096,performanceMode?1024:4096);sun.shadow.camera.left=-170;sun.shadow.camera.right=170;sun.shadow.camera.top=145;sun.shadow.camera.bottom=-145;sun.shadow.camera.near=80;sun.shadow.camera.far=1300;sun.shadow.bias=-.00018;sun.target.position.set(-250,0,0);scene.add(sun,sun.target);
 const coolFill=new THREE.DirectionalLight(0xb8d5df,.20);coolFill.position.set(480,260,520);coolFill.target.position.set(-250,20,0);scene.add(coolFill,coolFill.target);

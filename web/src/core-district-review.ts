...
function finalizeHeroMaterials(root:THREE.Object3D,performance=false){
 root.traverse(child=>{if(!(child instanceof THREE.Mesh)||Array.isArray(child.material))return;const current=child.material,name=current.name.toLowerCase();
  ...
  else if(name.includes('frontage-glass')||name.includes('pavilion-glass'))child.material=performance?new THREE.MeshStandardMaterial({name:current.name,color:0x315c65,roughness:.26,metalness:.08}):new THREE.MeshPhysicalMaterial({name:current.name,color:0x315c65,roughness:.2,metalness:.05,transmission:.18,transparent:true,opacity:.72,depthWrite:true,clearcoat:.45});
  ...
  else if(name.includes('archive-warm-stone'))child.material=new THREE.MeshStandardMaterial({name:current.name,color:0x9f9788,roughness:.73});
  else if(name.includes('ledger-limestone'))child.material=new THREE.MeshStandardMaterial({name:current.name,color:0x96948c,roughness:.67});
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const scene=new THREE.Scene();scene.background=new THREE.Color(0xb7c5cc);scene.fog=new THREE.Fog(0xb7c5cc,900,2200);
 ...
 const hemi=new THREE.HemisphereLight(0xe9f1f5,0x414943,1.15);scene.add(hemi);
 const sun=new THREE.DirectionalLight(0xffe3bb,3.35);sun.position.set(-520,760,-620);sun.castShadow=!performanceMode&&!!manifest.heroZones?.length;sun.shadow.mapSize.set(1536,1536);sun.shadow.camera.left=-260;sun.shadow.camera.right=260;sun.shadow.camera.top=220;sun.shadow.camera.bottom=-220;sun.shadow.camera.near=120;sun.shadow.camera.far=1500;sun.shadow.bias=-.00035;sun.target.position.set(-250,0,0);scene.add(sun,sun.target);
 ...
 animate();
 const setTime=(mode:'day'|'dusk'|'night')=>{if(mode==='night'){scene.background=new THREE.Color(0x111b2d);scene.fog=new THREE.Fog(0x111b2d,750,1900);scene.environmentIntensity=.24;renderer.toneMappingExposure=.86;hemi.intensity=.34;sun.intensity=.14;setHeroMaterialState(true);plazaLights.forEach(x=>x.intensity=performanceMode?0:75);heroLights.forEach(x=>x.intensity=performanceMode?0:95);streamLights.forEach(x=>x.light.intensity=performanceMode?0:x.intensity);}else if(mode==='dusk'){scene.background=new THREE.Color(0x766f7f);scene.fog=new THREE.Fog(0x766f7f,850,2050);scene.environmentIntensity=.58;renderer.toneMappingExposure=.98;hemi.intensity=1.1;sun.intensity=.85;setHeroMaterialState(true);plazaLights.forEach(x=>x.intensity=performanceMode?0:32);heroLights.forEach(x=>x.intensity=performanceMode?0:48);streamLights.forEach(x=>x.light.intensity=performanceMode?0:x.intensity*.45);}else{scene.background=new THREE.Color(0xb7c5cc);scene.fog=new THREE.Fog(0xb7c5cc,900,2200);scene.environmentIntensity=.82;renderer.toneMappingExposure=.96;hemi.intensity=1.15;sun.intensity=3.35;setHeroMaterialState(false);plazaLights.forEach(x=>x.intensity=0);heroLights.forEach(x=>x.intensity=0);streamLights.forEach(x=>x.light.intensity=0);}};

...
const cameras:CameraPreset[]=[
 ...
 {name:'hero-s-ledger-terrace',position:[18,3.95,-28],target:[60,4.2,40]},
 {name:'hero-s-ledger-frontage',position:[98,3.95,-28],target:[60,4.2,40]},
 {name:'hero-s-transit-junction',position:[218,3.95,-28],target:[260,4.6,40]},
 {name:'hero-s-transit-entry',position:[302,3.95,-28],target:[260,4.6,40]},
 {name:'hero-s-core-corridor',position:[118,3.95,-28],target:[158,4.2,40]},
 {name:'hero-s-east-gateway',position:[320,3.95,-28],target:[342,4.2,40]},
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const assetRevision='v13-wall-first-support-envelope-2',loaded=new Map<string,THREE.Object3D>();await Promise.all(manifest.families.map(async family=>{const gltf=await loader.loadAsync(`${base}/${family.lod[reviewLod]}?rev=${assetRevision}`);finalizeSupportFamilyMaterials(gltf.scene,family.id,performanceMode);loaded.set(family.id,gltf.scene);}));

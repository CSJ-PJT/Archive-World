...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string){
 ...
 const renderer=new THREE.WebGLRenderer({antialias:true,preserveDrawingBuffer:true});renderer.setPixelRatio(Math.min(devicePixelRatio,1));renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.72;renderer.shadowMap.enabled=true;renderer.shadowMap.autoUpdate=false;host.append(renderer.domElement);
 const controls=new OrbitControls(camera,renderer.domElement);controls.target.set(0,55,0);controls.enableDamping=true;
 const hemi=new THREE.HemisphereLight(0xeaf5ff,0x52614c,1.35);scene.add(hemi);
 const sun=new THREE.DirectionalLight(0xfff0d0,2);sun.position.set(-500,900,-350);scene.add(sun);
 ...
 animate();
 const setTime=(mode:'day'|'dusk'|'night')=>{if(mode==='night'){scene.background=new THREE.Color(0x111b2d);scene.fog=new THREE.Fog(0x111b2d,750,1900);renderer.toneMappingExposure=.85;hemi.intensity=.62;sun.intensity=.28;plazaLights.forEach(x=>x.intensity=95);}else if(mode==='dusk'){scene.background=new THREE.Color(0x766f7f);scene.fog=new THREE.Fog(0x766f7f,850,2050);renderer.toneMappingExposure=.78;hemi.intensity=.85;sun.intensity=.62;plazaLights.forEach(x=>x.intensity=40);}else{scene.background=new THREE.Color(0xaac2d0);scene.fog=new THREE.Fog(0xaac2d0,900,2200);renderer.toneMappingExposure=.72;hemi.intensity=1.35;sun.intensity=2;plazaLights.forEach(x=>x.intensity=0);}};

...
function finalizeHeroMaterials(root:THREE.Object3D,performance=false){
 root.traverse(child=>{if(!(child instanceof THREE.Mesh)||Array.isArray(child.material))return;const current=child.material,name=current.name.toLowerCase();
  ...
  if(name.includes('shallow-water'))child.material=pooledHeroMaterial(`water-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:'hero-water-performance',color:0x164b54,roughness:.40,metalness:.02,transparent:false}):new THREE.MeshPhysicalMaterial({name:'hero-water-cinematic',color:0x164b54,roughness:.32,metalness:.02,transmission:.08,thickness:.34,ior:1.333,transparent:true,opacity:.94,depthWrite:true,side:THREE.DoubleSide,clearcoat:.35,clearcoatRoughness:.28}));
  else if(name.includes('frontage-glass')||name.includes('pavilion-glass'))child.material=pooledHeroMaterial(`frontage-glass-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:'frontage-glass',color:0x29464d,roughness:.28,metalness:.08}):new THREE.MeshPhysicalMaterial({name:'frontage-glass',color:0x29464d,roughness:.16,metalness:.03,transmission:.38,transparent:true,opacity:.70,depthWrite:false,clearcoat:.62,clearcoatRoughness:.16}));
  else if(name.includes('occupied-window-glass'))child.material=pooledHeroMaterial('occupied-window-glass',()=>new THREE.MeshStandardMaterial({name:'occupied-window-glass',color:0x2e535c,roughness:.26,metalness:.08,emissive:0x9b5b25,emissiveIntensity:.08}));
  else if(name.includes('blue-gray-glass'))child.material=pooledHeroMaterial(`blue-gray-glass-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:'blue-gray-glass',color:0x17333c,roughness:.29,metalness:.10}):new THREE.MeshPhysicalMaterial({name:'blue-gray-glass',color:0x17333c,roughness:.16,metalness:.10,clearcoat:.68,clearcoatRoughness:.16}));
  else if(name.includes('archive-warm-stone'))child.material=pooledHeroMaterial('archive-warm-stone',()=>proceduralSurface(new THREE.MeshStandardMaterial({name:'archive-warm-stone',color:0x91897b,roughness:.73}),'archive-warm-stone',.07,.16));
  else if(name.includes('ledger-limestone'))child.material=pooledHeroMaterial('ledger-limestone',()=>proceduralSurface(new THREE.MeshStandardMaterial({name:'ledger-limestone',color:0x878b88,roughness:.67}),'ledger-limestone',.06,.18));
  ...
  else if(name.includes('dry-stone'))child.material=pooledHeroMaterial('dry-stone',()=>proceduralSurface(new THREE.MeshStandardMaterial({name:'dry-stone',color:0x696b67,roughness:.8}),'dry-stone',.045,.32));
  ...
  else if(name.includes('promenade-paver'))child.material=pooledHeroMaterial('promenade-paver',()=>proceduralSurface(new THREE.MeshStandardMaterial({name:'promenade-paver',color:0x5f635f,roughness:.88}),'promenade-paver',.055,.28));
  ...
  else if(name.includes('warm-interior'))child.material=pooledHeroMaterial('warm-interior',()=>new THREE.MeshStandardMaterial({name:'warm-interior',color:0x5f3b29,roughness:.64,emissive:0x7d3514,emissiveIntensity:1.15}));
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const scene=new THREE.Scene();scene.background=null;scene.fog=new THREE.Fog(0xc5d0d3,1200,2600);
 const skyUniforms={topColor:{value:new THREE.Color(0x7695aa)},bottomColor:{value:new THREE.Color(0xd2e1e5)},offset:{value:24.0},exponent:{value:.72}};
 ...
 const hemi=new THREE.HemisphereLight(0xe7eef2,0x27332f,.70);scene.add(hemi);
 const sun=new THREE.DirectionalLight(0xffd7ac,2.20);sun.position.set(-520,760,-620);sun.castShadow=!performanceMode&&!!manifest.heroZones?.length;sun.shadow.mapSize.set(1536,1536);sun.shadow.camera.left=-260;sun.shadow.camera.right=260;sun.shadow.camera.top=220;sun.shadow.camera.bottom=-220;sun.shadow.camera.near=120;sun.shadow.camera.far=1500;sun.shadow.bias=-.00035;sun.target.position.set(-250,0,0);scene.add(sun,sun.target);
 const coolFill=new THREE.DirectionalLight(0xa9c8d6,.26);coolFill.position.set(480,260,520);coolFill.target.position.set(-250,20,0);scene.add(coolFill,coolFill.target);
 ...
 const setHeroMaterialState=(night:boolean)=>heroZones.traverse(child=>{if(!(child instanceof THREE.Mesh)||Array.isArray(child.material))return;const mat=child.material as THREE.MeshStandardMaterial,name=mat.name.toLowerCase();if(name.includes('occupied-window-glass'))mat.emissiveIntensity=night?1.8:.03;else if(name.includes('warm-interior'))mat.emissiveIntensity=night?2.35:.08;else if(name.includes('warm-light'))mat.emissiveIntensity=night?1.7:.10;else if(name.includes('archive-cyan-light'))mat.emissiveIntensity=night?1.25:.08;});
 ...
 animate();
 const setTime=(mode:'day'|'dusk'|'night')=>{if(mode==='night'){skyUniforms.topColor.value.set(0x07111d);skyUniforms.bottomColor.value.set(0x24303b);skyUniforms.exponent.value=.86;scene.fog=new THREE.Fog(0x111b2d,750,1900);scene.environmentIntensity=.20;renderer.toneMappingExposure=.94;hemi.intensity=.20;sun.intensity=.08;coolFill.intensity=.10;setHeroMaterialState(true);plazaLights.forEach(x=>x.intensity=performanceMode?0:42);heroLights.forEach(x=>x.intensity=performanceMode?0:52);heroRouteLights.forEach(x=>x.intensity=performanceMode?0:18);heroPromenadeLights.forEach(x=>x.intensity=performanceMode?0:22);heroBridgeLights.forEach(x=>x.intensity=performanceMode?0:20);streamLights.forEach(x=>x.light.intensity=performanceMode?0:x.intensity*.58);}else if(mode==='dusk'){skyUniforms.topColor.value.set(0x574d6b);skyUniforms.bottomColor.value.set(0xd39b78);skyUniforms.exponent.value=.68;scene.fog=new THREE.Fog(0x8a7a83,850,2050);scene.environmentIntensity=.52;renderer.toneMappingExposure=.98;hemi.intensity=.56;sun.intensity=.72;coolFill.intensity=.22;setHeroMaterialState(true);plazaLights.forEach(x=>x.intensity=performanceMode?0:24);heroLights.forEach(x=>x.intensity=performanceMode?0:34);heroRouteLights.forEach(x=>x.intensity=performanceMode?0:11);heroPromenadeLights.forEach(x=>x.intensity=performanceMode?0:14);heroBridgeLights.forEach(x=>x.intensity=performanceMode?0:12);streamLights.forEach(x=>x.light.intensity=performanceMode?0:x.intensity*.32);}else{skyUniforms.topColor.value.set(0x7695aa);skyUniforms.bottomColor.value.set(0xd2e1e5);skyUniforms.exponent.value=.72;scene.fog=new THREE.Fog(0xc5d0d3,1200,2600);scene.environmentIntensity=.68;renderer.toneMappingExposure=.98;hemi.intensity=.70;sun.intensity=2.20;coolFill.intensity=.26;setHeroMaterialState(false);plazaLights.forEach(x=>x.intensity=0);heroLights.forEach(x=>x.intensity=0);heroRouteLights.forEach(x=>x.intensity=0);heroPromenadeLights.forEach(x=>x.intensity=0);heroBridgeLights.forEach(x=>x.intensity=0);streamLights.forEach(x=>x.light.intensity=0);}};

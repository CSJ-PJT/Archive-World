...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const lightingTarget=new THREE.Vector3(...selected.target);const sun=new THREE.DirectionalLight(0xffe2bc,3.15);sun.position.copy(lightingTarget).add(new THREE.Vector3(-310,520,-370));sun.castShadow=!performanceMode&&!!manifest.heroZones?.length;sun.shadow.mapSize.set(performanceMode?1024:4096,performanceMode?1024:4096);sun.shadow.camera.left=-230;sun.shadow.camera.right=230;sun.shadow.camera.top=180;sun.shadow.camera.bottom=-180;sun.shadow.camera.near=80;sun.shadow.camera.far=1300;sun.shadow.bias=-.00018;sun.target.position.copy(lightingTarget);scene.add(sun,sun.target);

...
const cameras:CameraPreset[]=[
 ...
 {name:'hero-s-ledger-terrace',position:[41,4.45,-18],target:[82,4.05,8]},
 {name:'hero-s-ledger-frontage',position:[96,4.05,-16],target:[58,4.15,10]},
 {name:'hero-s-transit-junction',position:[208,4.15,-14],target:[267,4.00,7]},
...
function finalizeHeroMaterials(root:THREE.Object3D,performance=false){
 root.traverse(child=>{if(!(child instanceof THREE.Mesh)||Array.isArray(child.material))return;const current=child.material,name=current.name.toLowerCase();
  ...
  else if(name.includes('frontage-glass')||name.includes('pavilion-glass'))child.material=pooledHeroMaterial(`frontage-glass-${performance}`,()=>proceduralGlazing(performance?new THREE.MeshStandardMaterial({name:'frontage-glass',color:0x183b46,roughness:.24,metalness:.08}):new THREE.MeshPhysicalMaterial({name:'frontage-glass',color:0x214854,roughness:.14,metalness:.03,transmission:.28,transparent:true,opacity:.78,depthWrite:true,clearcoat:.84,clearcoatRoughness:.10,ior:1.46,thickness:.34}),'frontage',.08));
  ...
  else if(name.includes('archive-warm-stone'))child.material=pooledHeroMaterial('archive-warm-stone',()=>proceduralSurface(new THREE.MeshStandardMaterial({name:'archive-warm-stone',color:0xa89478,roughness:.66}),'archive-warm-stone',.095,.16));
  else if(name.includes('ledger-limestone'))child.material=pooledHeroMaterial('ledger-limestone',()=>proceduralSurface(new THREE.MeshStandardMaterial({name:'ledger-limestone',color:0x8b8a82,roughness:.60}),'ledger-limestone',.085,.18));
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 if(manifest.heroZones){for(const zone of manifest.heroZones.filter(includePerformanceHero)){const heroAssetRevision=`${zone.status}-g${zone.geometryRevision??0}-${zone.geometry?.triangles??'geometry'}-${performanceMode?'lod2-v4':'hero'}`,heroUri=performanceMode&&zone.performanceUri?zone.performanceUri:zone.uri;const hero=await loader.loadAsync(`${base}/${heroUri}?rev=${encodeURIComponent(heroAssetRevision)}`);finalizeHeroMaterials(hero.scene,performanceMode);if(performanceMode&&needsPerformanceConsolidation(hero.scene))consolidatePerformanceRoot(hero.scene);hero.scene.userData={heroZone:zone.id,status:zone.status,actual3D:true,performanceLod:performanceMode,preconsolidatedByMaterial:true};heroZones.add(hero.scene);loadedHeroZoneIds.push(zone.id);if(performanceMode)await new Promise<void>(resolve=>setTimeout(resolve,0));}}

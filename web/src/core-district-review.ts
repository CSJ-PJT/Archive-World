...
function finalizeHeroMaterials(root:THREE.Object3D,performance=false){
 root.traverse(child=>{if(!(child instanceof THREE.Mesh)||Array.isArray(child.material))return;const current=child.material,name=current.name.toLowerCase();
  ...
  else if(name.includes('frontage-glass')||name.includes('pavilion-glass'))child.material=pooledHeroMaterial(`frontage-glass-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:'frontage-glass',color:0x284953,roughness:.27,metalness:.08}):new THREE.MeshPhysicalMaterial({name:'frontage-glass',color:0x345966,roughness:.16,metalness:.02,transmission:.30,transparent:true,opacity:.76,depthWrite:false,clearcoat:.62,clearcoatRoughness:.16,ior:1.46,thickness:.18}));
  else if(name.includes('occupied-window-glass'))child.material=pooledHeroMaterial('occupied-window-glass',()=>new THREE.MeshPhysicalMaterial({name:'occupied-window-glass',color:0x234a55,roughness:.19,metalness:.05,clearcoat:.58,clearcoatRoughness:.14,emissive:0xa45b23,emissiveIntensity:.08}));
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const scene=new THREE.Scene();scene.background=null;scene.fog=new THREE.Fog(0xb9c7cb,1300,2750);
 const skyUniforms={topColor:{value:new THREE.Color(0x6d91aa)},bottomColor:{value:new THREE.Color(0xd8e2df)},offset:{value:24.0},exponent:{value:.68}};
 ...
 const hemi=new THREE.HemisphereLight(0xe7eef2,0x25312e,.22);scene.add(hemi);
 const sun=new THREE.DirectionalLight(0xffd4a4,2.65);sun.position.set(-430,620,-520);sun.castShadow=!performanceMode&&!!manifest.heroZones?.length;sun.shadow.mapSize.set(2048,2048);sun.shadow.camera.left=-250;sun.shadow.camera.right=250;sun.shadow.camera.top=190;sun.shadow.camera.bottom=-190;sun.shadow.camera.near=80;sun.shadow.camera.far=1300;sun.shadow.bias=-.00022;sun.target.position.set(-250,0,0);scene.add(sun,sun.target);
 ...
 const groundMaterial=new THREE.MeshStandardMaterial({color:0x46514c,roughness:.96});
 ...
 animate();
 const setTime=(mode:'day'|'dusk'|'night')=>{if(mode==='night'){skyUniforms.topColor.value.set(0x07111d);skyUniforms.bottomColor.value.set(0x24303b);skyUniforms.exponent.value=.86;scene.fog=new THREE.Fog(0x111b2d,750,1900);scene.environmentIntensity=.26;renderer.toneMappingExposure=1.02;hemi.intensity=.28;sun.intensity=.10;coolFill.intensity=.14;setHeroMaterialState(true);plazaLights.forEach(x=>x.intensity=performanceMode?0:32);heroLights.forEach(x=>x.intensity=performanceMode?0:28);heroRouteLights.forEach(x=>x.intensity=performanceMode?0:16);heroPromenadeLights.forEach(x=>x.intensity=performanceMode?0:19);heroBridgeLights.forEach(x=>x.intensity=performanceMode?0:18);streamLights.forEach(x=>x.light.intensity=performanceMode?0:x.intensity*.48);}else if(mode==='dusk'){skyUniforms.topColor.value.set(0x574d6b);skyUniforms.bottomColor.value.set(0xd39b78);skyUniforms.exponent.value=.68;scene.fog=new THREE.Fog(0x8a7a83,850,2050);scene.environmentIntensity=.42;renderer.toneMappingExposure=.98;hemi.intensity=.42;sun.intensity=.88;coolFill.intensity=.18;setHeroMaterialState(true);plazaLights.forEach(x=>x.intensity=performanceMode?0:24);heroLights.forEach(x=>x.intensity=performanceMode?0:24);heroRouteLights.forEach(x=>x.intensity=performanceMode?0:11);heroPromenadeLights.forEach(x=>x.intensity=performanceMode?0:14);heroBridgeLights.forEach(x=>x.intensity=performanceMode?0:12);streamLights.forEach(x=>x.light.intensity=performanceMode?0:x.intensity*.32);}else{skyUniforms.topColor.value.set(0x6d91aa);skyUniforms.bottomColor.value.set(0xd8e2df);skyUniforms.exponent.value=.68;scene.fog=new THREE.Fog(0xb9c7cb,1300,2750);scene.environmentIntensity=.34;renderer.toneMappingExposure=.94;hemi.intensity=.22;sun.intensity=2.65;coolFill.intensity=.10;setHeroMaterialState(false);plazaLights.forEach(x=>x.intensity=0);heroLights.forEach(x=>x.intensity=0);heroRouteLights.forEach(x=>x.intensity=0);heroPromenadeLights.forEach(x=>x.intensity=0);heroBridgeLights.forEach(x=>x.intensity=0);streamLights.forEach(x=>x.light.intensity=0);}};

...
const cameras:CameraPreset[]=[
 ...
 {name:'hero-s-ledger-terrace',position:[22,3.95,28],target:[75,4.2,-40]},
 {name:'hero-s-ledger-frontage',position:[98,3.95,-28],target:[60,4.2,40]},
 {name:'hero-s-transit-junction',position:[220,3.95,28],target:[274,4.4,-40]},
 {name:'hero-s-transit-entry',position:[302,3.95,-28],target:[260,4.6,40]},
 {name:'hero-s-core-corridor',position:[132,3.95,28],target:[190,3.0,-20]},
 {name:'hero-s-east-gateway',position:[345,3.95,-28],target:[300,3.4,30]},
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const assetRevision='v13-wall-first-support-envelope-1',loaded=new Map<string,THREE.Object3D>();await Promise.all(manifest.families.map(async family=>{const gltf=await loader.loadAsync(`${base}/${family.lod[reviewLod]}?rev=${assetRevision}`);finalizeSupportFamilyMaterials(gltf.scene,family.id,performanceMode);loaded.set(family.id,gltf.scene);}));

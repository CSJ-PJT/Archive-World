...
function proceduralWaterMaterial(performance=false){
 const water=new THREE.MeshStandardMaterial({name:performance?'hero-water-performance':'hero-water-cinematic',color:0x155764,roughness:performance?.48:.36,metalness:.04,transparent:false});
 if(!performance){water.onBeforeCompile=shader=>{shader.vertexShader=shader.vertexShader.replace('#include <common>','#include <common>\nvarying vec3 vArchiveWaterWorld;').replace('#include <worldpos_vertex>','#include <worldpos_vertex>\nvArchiveWaterWorld=(modelMatrix*vec4(transformed,1.0)).xyz;');shader.fragmentShader=shader.fragmentShader.replace('#include <common>','#include <common>\nvarying vec3 vArchiveWaterWorld;').replace('#include <normal_fragment_maps>','#include <normal_fragment_maps>\nfloat streamWaveA=sin(vArchiveWaterWorld.x*.72+vArchiveWaterWorld.z*1.18);float streamWaveB=cos(vArchiveWaterWorld.x*1.31-vArchiveWaterWorld.z*.83);normal=normalize(normal+vec3(streamWaveA*.055,0.0,streamWaveB*.045));').replace('#include <color_fragment>','#include <color_fragment>\nfloat streamFlow=.92+.08*sin(vArchiveWaterWorld.x*.24+vArchiveWaterWorld.z*.65);diffuseColor.rgb*=streamFlow;');};water.customProgramCacheKey=()=> 'archive-procedural-water-v1';}
 return water;
}
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const hemi=new THREE.HemisphereLight(0xe7eef2,0x27332f,.46);scene.add(hemi);
 const sun=new THREE.DirectionalLight(0xffd7ac,2.75);sun.position.set(-520,760,-620);sun.castShadow=!performanceMode&&!!manifest.heroZones?.length;sun.shadow.mapSize.set(1536,1536);sun.shadow.camera.left=-260;sun.shadow.camera.right=260;sun.shadow.camera.top=220;sun.shadow.camera.bottom=-220;sun.shadow.camera.near=120;sun.shadow.camera.far=1500;sun.shadow.bias=-.00035;sun.target.position.set(-250,0,0);scene.add(sun,sun.target);
 const coolFill=new THREE.DirectionalLight(0xa9c8d6,.18);coolFill.position.set(480,260,520);coolFill.target.position.set(-250,20,0);scene.add(coolFill,coolFill.target);

...
function finalizeSupportFamilyMaterials(root:THREE.Object3D,familyId:string,performance=false){
 ...
 root.traverse(child=>{if(!(child instanceof THREE.Mesh)||Array.isArray(child.material))return;const current=child.material,name=current.name.toLowerCase();child.castShadow=!performance;child.receiveShadow=true;
  if(name.includes('occupied-window-glass'))child.material=pooledSupportMaterial(`occupied-window-${tone}-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:`support-occupied-window-${tone}`,color:[0x315562,0x425a60,0x304f59][tone],roughness:.22,metalness:.08,emissive:0xc46b2c,emissiveIntensity:.03}):new THREE.MeshPhysicalMaterial({name:`support-occupied-window-${tone}`,color:[0x294d5a,0x3c555d,0x294c56][tone],roughness:.18,metalness:.03,clearcoat:.76,clearcoatRoughness:.14,emissive:0xc46b2c,emissiveIntensity:.03}));
  else if(name.includes('curtain-wall-glass')||name.includes('residential-glass')||name.includes('blue-gray-glass'))child.material=pooledSupportMaterial(`glass-${tone}-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:`support-glass-${tone}`,color:[0x315562,0x425a60,0x304f59][tone],roughness:.24,metalness:.12}):new THREE.MeshPhysicalMaterial({name:`support-glass-${tone}`,color:[0x284d59,0x3d555c,0x2b4c56][tone],roughness:.16,metalness:.03,clearcoat:.82,clearcoatRoughness:.13}));
...
function proceduralWaterMaterial(performance=false){
 const water=performance
  ...
  :new THREE.MeshPhysicalMaterial({name:'hero-water-cinematic',color:0x0f4b59,roughness:.16,metalness:0,transmission:.16,transparent:true,opacity:.86,depthWrite:true,clearcoat:.74,clearcoatRoughness:.12,ior:1.333,thickness:.45});
...
function finalizeHeroMaterials(root:THREE.Object3D,performance=false){
 root.traverse(child=>{if(!(child instanceof THREE.Mesh)||Array.isArray(child.material))return;const current=child.material,name=current.name.toLowerCase();
  ...
  else if(name.includes('frontage-glass')||name.includes('pavilion-glass'))child.material=pooledHeroMaterial(`frontage-glass-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:'frontage-glass',color:0x24444d,roughness:.25,metalness:.08}):new THREE.MeshPhysicalMaterial({name:'frontage-glass',color:0x31545c,roughness:.18,metalness:.01,transmission:.72,transparent:true,opacity:.38,depthWrite:false,clearcoat:.62,clearcoatRoughness:.16,ior:1.46,thickness:.22}));
  ...
  else if(name.includes('blue-gray-glass'))child.material=pooledHeroMaterial(`blue-gray-glass-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:'blue-gray-glass',color:0x294c58,roughness:.29,metalness:.10}):new THREE.MeshPhysicalMaterial({name:'blue-gray-glass',color:0x284f5c,roughness:.16,metalness:.04,clearcoat:.84,clearcoatRoughness:.13}));
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 const performanceFocus=new THREE.Vector3(...selected.target),performanceRadiusM=reviewHeroCamera?300:Infinity;
 ...
 if(composer){composer.addPass(new RenderPass(scene,camera));const gtao=new GTAOPass(scene,camera,window.innerWidth,window.innerHeight);gtao.updateGtaoMaterial({radius:.62,distanceExponent:1.62,thickness:.42,distanceFallOff:1,scale:1,samples:24});gtao.updatePdMaterial({samples:10,radius:2,radiusExponent:2});gtao.blendIntensity=.24;composer.addPass(gtao);const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),.09,.18,.94);composer.addPass(bloom);composer.addPass(new OutputPass());}
 ...
 const assetRevision='v17-metropolitan-envelope-datum',loaded=new Map<string,THREE.Object3D>();await Promise.all(manifest.families.map(async family=>{const gltf=await loader.loadAsync(`${base}/${family.lod[reviewLod]}?rev=${assetRevision}`);finalizeSupportFamilyMaterials(gltf.scene,family.id,performanceMode);loaded.set(family.id,gltf.scene);}));
 ...
 if(manifest.heroZones){for(const zone of manifest.heroZones.filter(includePerformanceHero)){const heroAssetRevision=`${zone.status}-g${zone.geometryRevision??0}-${zone.geometry?.triangles??'geometry'}-${performanceMode?'lod2-v3':'hero'}`,heroUri=performanceMode&&zone.performanceUri?zone.performanceUri:zone.uri;const hero=await loader.loadAsync(`${base}/${heroUri}?rev=${encodeURIComponent(heroAssetRevision)}`);finalizeHeroMaterials(hero.scene,performanceMode);hero.scene.userData={heroZone:zone.id,status:zone.status,actual3D:true,performanceLod:performanceMode,preconsolidatedByMaterial:true};heroZones.add(hero.scene);loadedHeroZoneIds.push(zone.id);if(performanceMode)await new Promise<void>(resolve=>setTimeout(resolve,0));}}

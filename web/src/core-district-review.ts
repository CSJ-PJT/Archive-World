...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string){
 ...
 const renderer=new THREE.WebGLRenderer({antialias:true,preserveDrawingBuffer:true});renderer.setPixelRatio(Math.min(devicePixelRatio,1));renderer.shadowMap.enabled=true;renderer.shadowMap.autoUpdate=false;host.append(renderer.domElement);
 const controls=new OrbitControls(camera,renderer.domElement);controls.target.set(0,55,0);controls.enableDamping=true;
 const hemi=new THREE.HemisphereLight(0xeaf5ff,0x52614c,2.2);scene.add(hemi);
 const sun=new THREE.DirectionalLight(0xfff0d0,3);sun.position.set(-500,900,-350);scene.add(sun);
 ...
 for(const family of manifest.families){const source=loaded.get(family.id);if(!source)continue;source.updateMatrixWorld(true);const items=manifest.instances.filter(item=>item.familyId===family.id);source.traverse(child=>{if(!(child instanceof THREE.Mesh))return;const mesh=new THREE.InstancedMesh(child.geometry,child.material,items.length);mesh.name=`batch-${family.id}-${child.name}`;mesh.userData={familyId:family.id,status:'ACTUAL_GLTF_INSTANCED'};items.forEach((item,index)=>{const matrix=new THREE.Matrix4().compose(new THREE.Vector3(...item.position),new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0,1,0),item.rotationY),new THREE.Vector3(item.scale,item.scale,item.scale));mesh.setMatrixAt(index,matrix.multiply(child.matrixWorld));});mesh.instanceMatrix.needsUpdate=true;buildings.add(mesh);});}
 ...
 animate();
 const setTime=(mode:'day'|'dusk'|'night')=>{if(mode==='night'){scene.background=new THREE.Color(0x111b2d);scene.fog=new THREE.Fog(0x111b2d,750,1900);hemi.intensity=.82;sun.intensity=.38;plazaLights.forEach(x=>x.intensity=150);}else if(mode==='dusk'){scene.background=new THREE.Color(0x766f7f);scene.fog=new THREE.Fog(0x766f7f,850,2050);hemi.intensity=1.15;sun.intensity=.95;plazaLights.forEach(x=>x.intensity=55);}else{scene.background=new THREE.Color(0xaac2d0);scene.fog=new THREE.Fog(0xaac2d0,900,2200);hemi.intensity=2.2;sun.intensity=3;plazaLights.forEach(x=>x.intensity=0);}};

...
function finalizeSupportFamilyMaterials(root:THREE.Object3D,familyId:string,performance=false){
 ...
 root.traverse(child=>{if(!(child instanceof THREE.Mesh)||Array.isArray(child.material))return;const current=child.material,name=current.name.toLowerCase();child.castShadow=!performance;child.receiveShadow=true;
  if(name.includes('occupied-window-glass'))child.material=pooledSupportMaterial(`occupied-window-${tone}-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:`support-occupied-window-${tone}`,color:[0x244853,0x334b52,0x263f49][tone],roughness:.22,metalness:.08,emissive:0xc46b2c,emissiveIntensity:.03}):new THREE.MeshPhysicalMaterial({name:`support-occupied-window-${tone}`,color:[0x1c3d48,0x2b444c,0x203844][tone],roughness:.15,metalness:.04,clearcoat:.70,clearcoatRoughness:.12,emissive:0xc46b2c,emissiveIntensity:.03}));
  else if(name.includes('curtain-wall-glass')||name.includes('residential-glass')||name.includes('blue-gray-glass'))child.material=pooledSupportMaterial(`glass-${tone}-${performance}`,()=>performance?new THREE.MeshStandardMaterial({name:`support-glass-${tone}`,color:[0x244853,0x334b52,0x263f49][tone],roughness:.24,metalness:.12}):new THREE.MeshPhysicalMaterial({name:`support-glass-${tone}`,color:[0x183946,0x29434b,0x1e3942][tone],roughness:.13,metalness:.04,clearcoat:.76,clearcoatRoughness:.11}));
...
export async function createCoreDistrictReview(app:HTMLDivElement,base:string,manifestFile='core-district-3d.json'){
 ...
 if(composer){composer.addPass(new RenderPass(scene,camera));const gtao=new GTAOPass(scene,camera,window.innerWidth,window.innerHeight);gtao.updateGtaoMaterial({radius:1.18,distanceExponent:1.48,thickness:.68,distanceFallOff:1,scale:1,samples:28});gtao.updatePdMaterial({samples:12,radius:3,radiusExponent:2});gtao.blendIntensity=.58;composer.addPass(gtao);const bloom=new UnrealBloomPass(new THREE.Vector2(window.innerWidth,window.innerHeight),.075,.16,.95);composer.addPass(bloom);composer.addPass(new OutputPass());}

...
def apply_palette(materials,seed):
 ...
 targets={'limestone':accent,'curtain-wall-glass':tuple(x*.52 for x in accent),'dark-metal-panel':tuple(x*.22 for x in accent),'light-metal-panel':tuple(x*.78 for x in accent)}
 ...
 return {'palette':materials['limestone']['districtPalette'],'accent':accent,'proceduralOnly':True}
